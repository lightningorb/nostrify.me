...
zoo`.split('\n');
	...
	var require_utils = __commonJS({
		'node_modules/@scure/bip39/node_modules/@noble/hashes/utils.js'(exports) {
			...
			function hexToBytes3(hex2) {
				...
				if (hex2.length % 2) throw new Error('hexToBytes: received invalid unpadded hex');
	...
	var require_sha256 = __commonJS({
		'node_modules/@scure/bip39/node_modules/@noble/hashes/sha256.js'(exports) {
			...
			var SHA2563 = class extends _sha2_js_1.SHA2 {
				...
				process(view, offset) {
					for (let i = 0; i < 16; i++, offset += 4) SHA256_W3[i] = view.getUint32(offset, false);
					for (let i = 16; i < 64; i++) {
	...
	var require_lib = __commonJS({
		'node_modules/@scure/base/lib/index.js'(exports) {
			...
			var base58check3 = (sha2563) =>
				chain2(
					checksum2(4, (data) => sha2563(sha2563(data))),
					exports.base58
				);
	...
	var require_bip39 = __commonJS({
		'node_modules/@scure/bip39/index.js'(exports) {
			...
			function mnemonicToSeedSync2(mnemonic, passphrase = '') {
				return (0, pbkdf2_1.pbkdf2)(sha512_1.sha512, normalize2(mnemonic).nfkd, salt(passphrase), {
					c: 2048,
					dkLen: 64
				});
			}
	...
	var Point = class {
		...
		static fromHex(hex2) {
			...
			if (len === 65 && header === 4) return this.fromUncompressedHex(bytes2);
			throw new Error(
	...
	function hexToBytes(hex2) {
		...
		if (hex2.length % 2) throw new Error('hexToBytes: received invalid unpadded hex' + hex2.length);
		const array = new Uint8Array(hex2.length / 2);
		for (let i = 0; i < array.length; i++) {
			const j = i * 2;
			const hexByte = hex2.slice(j, j + 2);
	...
	function invert(number2, modulo = CURVE.P) {
		...
		const gcd2 = b;
		if (gcd2 !== _1n) throw new Error('invert: does not exist');
		return mod(x, modulo);
	}
	...
	var utils = {
		...
		hmacSha256: async (key, ...messages) => {
			if (crypto2.web) {
				const ckey = await crypto2.web.subtle.importKey(
					'raw',
					key,
	...
	function verifySignature(event) {
		return schnorr.verifySync(event.sig, getEventHash(event), event.pubkey);
	}
	...
	function relayInit(url) {
		...
		const sub = (
			filters,
			{ skipVerification = false, id = Math.random().toString().slice(2) } = {}
		) => {
			let subid = id;
			openSubs[subid] = {
				id: subid,
	...
	var base58check = (sha2563) =>
		chain(
			checksum(4, (data) => sha2563(sha2563(data))),
			base58
		);
	...
	var HDKey = class {
		constructor(opt) {
			...
			if (opt.privateKey) {
				...
				this.privKey =
					typeof opt.privateKey === 'bigint' ? opt.privateKey : bytesToNumber2(opt.privateKey);
		...
		get privateExtendedKey() {
			...
			return base58check2.encode(
				this.serialize(this.versions.private, concatBytes2(new Uint8Array([0]), priv))
			);
		}
		...
		deriveChild(index) {
			...
			try {
				if (this.privateKey) {
					...
					opt.publicKey = Point.fromHex(this.pubKey)
						.add(Point.fromPrivateKey(childTweak))
						.toRawBytes(true);
				}
	...
	function getDelegator(event) {
		...
		if (!schnorr.verifySync(sig, sighash, pubkey)) return null;
		return pubkey;
	}
